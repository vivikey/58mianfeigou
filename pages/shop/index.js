...
var pageObj = {
    ...
    initDynamicList(){
        let dynamic_list = []
        for(let i=1;i<11;i++){
            dynamic_list.push({ id: i, user: "L**V", value: (Math.random()*10).toFixed(1),type:"佣金"})
        }
        this.setData({ dynamic_list: dynamic_list})
    },
    //-- 初始化导航数据
    initClassNavList(){
        let classNavList = [{ id: -1, categoryName: '推荐' }, { id: -2, categoryName: '热门' }, { id: -3, categoryName: '上新' }]
        Shop.TypeList({ class_id:0}).then(r=>{
            console.log('Shop.TypeList => ',r)
            r.data.forEach(item=>{
                classNavList.push({ id: item.id, categoryName: item.categoryName})
            })
            this.setData({
